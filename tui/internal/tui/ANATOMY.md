...
- **`firstrun.go:135-1989`** (4150 lines) — `FirstRunModel`. Multi-step wizard with constructors for three flows: `NewFirstRunModel` (new project, `firstrun.go:283`), `NewSetupModeModel` (reconfiguring an existing agent, `firstrun.go:452`), `NewRehydrateModel` (cloned network, `firstrun.go:548`). Its background bootstrap calls `config.EnsureRuntimeQuiet`, so first-run venv creation is followed by the same non-blocking Python `lingtai` upgrade check as returning-user startup. Steps (`firstRunStep` enum, `firstrun.go:63-76`): Welcome → API Key → Pick Preset → Edit Preset → Preset Key → Capabilities → Agent Presets → Agent Name/Dir → Recipe → (optional) Rehydrate Propagate → Launching. Emits `FirstRunDoneMsg` when complete.
...
- **`library.go:532-906`** — `LibraryModel`. Skill catalog browser, agent-scoped — scans `<agent>/.library/` plus all `skills.paths` from `init.json`. Constructor: `NewLibraryModel` (`library.go:594`). Renders skill metadata in a sidebar list with markdown content pane.
